const fs = require(`fs`);
const data_restricted = fs.readFileSync(`./config_vip.json`);


// Четене на JSON файла
const data_vip = fs.readFileSync(`./config_vip.json`);
const words = JSON.parse(data_vip);
class Person {
    function() {
...
// Добавяне на приятел
const addNewFriend =  fs.readFileSync(`./config_vip.json`);
...
// Добавяне на снимки
const addNewPhoto =  fs.readFileSync(`./config_vip.json`);
const filePhoto = addNewPhoto + ``;
const jsObjectPhoto = JSON.parse(filePhoto);
console.log(jsObjectPhoto);

// Добавяне на видео 
const addNewVideo =  fs.readFileSync(`./config_vip.json`);
